...
class WAVE(object):
    ...
    def cal(self):
        ...
        for i in range(1,self.M,1):
           self.y1[i]=0.03*(math.exp(-1000.*((i*self.dx-1.0)**2))+math.exp(-1000.*((i*self.dx-0.5)**2))+math.exp(-1000.*((i*self.dx)**2)))
    ...
    def plot(self,_t):
        ...
        self.ax.plot(self.x,self.y2,'r-',label='time = %.2f (ms)'%_t)
        self.ax.set_xlim(-0.0,1.0)
        self.ax.set_ylim(-0.06,0.08)
        self.ax.set_xlabel("x (m)",fontsize=18)
        self.ax.set_ylabel("y (m)",fontsize=18)
        self.ax.set_title('Wave on a string: Two Packets',fontsize=18)
        self.ax.legend(loc='best',fontsize=14)
        plt.savefig(r'd:\tmp\save'+'%.2f'%_t+r'.jpg')
