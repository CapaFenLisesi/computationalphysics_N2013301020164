...
a_ttl=pendulum_gen(0.5)
b_ttl=pendulum_gen1(0.5)
c_ttl=pendulum_gen(1.2)
d_ttl=pendulum_gen1(1.2)

delta_th1=[0.001]
delta_th2=[0.001]
loop_1=0
while loop_1<N:
    delta_th1.append(abs(a_ttl[0][loop_1]-b_ttl[0][loop_1]))
    delta_th2.append(abs(c_ttl[0][loop_1]-d_ttl[0][loop_1]))
    loop_1=loop_1+1
...
plt.figure(1)
plt.subplot(211)
line_1,=plt.plot(t_e,delta_th1,'r-')
plt.xlabel('t(s)')
plt.ylabel('$\\Delta \\theta$')
plt.title('Deviation Evolution')
plt.legend([line_1],['$F_D=0.5$'],fontsize='x-small')

plt.subplot(212)
line_1,=plt.plot(t_e,delta_th2,'r-')
plt.xlabel('t(s)')
plt.ylabel('$\\Delta \\theta$')
